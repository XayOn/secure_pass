...
--site_type <SITE_TYPE> --username <USERNAME>
    ...
    --newkey <NEW_KEY>
    keystore --path <PATH> --recipient <RECIPIENT> --change_password_random \
    --site_name <NAME> --site_type <SITE_TYPE> --username <USERNAME>
    keystore --path <PATH> --recipient <RECIPIENT> --save --site_name <NAME> \
--site_type <SITE_TYPE> --username <USERNAME> --newkey <KEY>
    keystore --path <PATH> --recipient <RECIPIENT> --save_random \
    --site_name <NAME> --site_type <SITE_TYPE> --username <USERNAME>
...
Options:
    ...
    --get                    Return a saved password
    --save                   Save a password
    --save_random            Save a random_password
    --autogen                Save an autogenerated password
    --delete                 Delete a password
    --login                  Open a browser and log in
    --logout                 Open a browser and log-out
    --change_password        Open a browser and change password
    --change_password_random Open a browser and change password to a rnd one
    --newkey=<key>           New key for password change / creation
...
class Site:
    ...
    def save(self, username, newkey):
        """ Save a new key """
        self.keys[username].key = newkey

    def save_random(self, username):
        """ Save a new random key """
    ...
    def change_password_random(self, username):
        """ Use selenium to change password to a random one """
        if not self.browser:
            assert RuntimeError("Cant use browser methods "
                                "without specifying a site type")

        oldkey = self.keys[username].key
        self.save_random(username)

        return self.automation.change_password(
            username, oldkey, self.keys[username].key)
